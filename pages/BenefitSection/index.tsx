        ...
        <>
            <section className="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div className="p-12 bg-white flex justify-center items-center">
                    <div>
                        <Image src={"/benefitImage.webp"} width={500} height={500} alt="Bocil VR" className="bg-cover"/>
                    </div>
                </div>
                <div>
                    <h1 className="font-bold text-5xl max-w-lg">
                        Benefits of Educating Yourself in a World
                    ...
                    <ul className="grid grid-cols-2 gap-4 mt-6">
                        { BenefitInputData.map((input, id) => (
                            <li key={id} className="font-medium">{id+1}.{input.title}</li>
                        ))}
                    </ul>
                </div>
            </section>
